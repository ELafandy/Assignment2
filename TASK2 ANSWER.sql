...
WHERE state IN ('CA','NY'); --WHERE state ='CA'OR state ='NY';

-- Retrieve all orders place1d in 2023
SELECT * 
FROM sales.orders
WHERE YEAR(order_date) = 2023;--YEAR(VARIABLE) MEANS YEAR FROM ADDED DATE 
...
SELECT brand_id,
AVG(list_price) AS average_price --AVG(Variable) Means average of the sum,
FROM production.products
GROUP BY brand_id;
